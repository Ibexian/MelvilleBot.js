...
function stringPretty(string) {
	...
	var endNum =  reg.test(string.charAt(string.length-2)) ? string.length : string.length - 1;
	//Start with a capital, remove the final space, and add punctuation if there is none
    return string.charAt(0).toUpperCase() + string.slice(1, endNum) + (endNum === string.length ? "" : ".");
}
